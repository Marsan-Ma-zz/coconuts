...
(function() {
  ...
  $(window).scroll(function(){
    ...
    if(nearBottomOfPage()) {
      var bdata = $("#infinite_chips").attr("bdata");
      var btype = $("#infinite_chips").attr("btype");
      var bname = $("#infinite_chips").attr("bname");
      var blang = $("#infinite_chips").attr("blang");
      if(bname != "ending") {
        loading=true;
        page++;
        $("#loading_icon").show();
        $.ajax({
          url: '/' + bdata + '?' + btype + '=' + bname + '&lang=' + blang + '&page=' + page,
          type: 'get',
          dataType: 'script',
          success: function() {
            $(window).sausage('draw');
