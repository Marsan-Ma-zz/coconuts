...
(function($) {
	...
	$.eAccordion = function(el, options) {
		...
		base.init = function() {
			...
			base.contractedWidth = (base.$el.width() - expandedWidth*1.05) / (base.pages - 1);

      //on Window-size changed		
      var waitForChangeDone = (function () {
        var timers = {};
		...
		base.gotoPage = function(page, animate) {
			...
			$page.children('.heading').stop(true, true).animate({opacity: 0,'width':base.options.expandedWidth}, duration);
			$page.children('.description, .bgDescription').stop(true, true).animate({
        width: '100%',
				opacity: 0.7,
				marginBottom: 0,
				paddingBottom: 0
			...
			$siblings.children('.heading').stop(true, true).animate({opacity: 0.6, 'width':base.contractedWidth}, duration);
			$siblings.children('.description, .bgDescription').stop(true, true).animate({
				opacity: 0,
		...
		base.gotoNeutral = function(animate){
			...
			base.$items.children('.heading').stop(true, true).animate({opacity: .6}, duration);
			base.$items.children('.description, .bgDescription').stop(true, true).animate({
				opacity: 0,
				marginBottom: '-' + base.options.bgHeight,
				paddingBottom: base.options.bgHeight
			}, duration);
	...
	$.eAccordion.defaults = {
		...
		expandedWidth: '80%',			// Width of the expanded slide
